...
must_haves:
  ...
  key_links:
    ...
    - from: "apps/omnii_mcp/src/services/integrations/n8n-workflow-client.ts"
      ...
      via: "backOff wrapper on API calls"
      pattern: "backOff\\(\\(\\) =>"
---
...
1. Create `apps/omnii_mcp/src/services/integrations/n8n-workflow-client.ts`:
   ...
   c. Create N8nWorkflowClient class:
      ...
      - `async listWorkflows(): Promise<Workflow[]>`
        Implement with backOff retry wrapper:
        ```typescript
        async listWorkflows(): Promise<Workflow[]> {
          await logAuditEvent({
          ...
          try {
            const response = await backOff(
              () => fetch(`${this.baseUrl}/api/v1/workflows`, {
              ...
              }),
              {
                ...
                retry: (err, attemptNumber) => {
                  console.warn(`[N8nWorkflowClient] listWorkflows attempt ${attemptNumber} failed:`, err.message);
            ...
            console.error('[N8nWorkflowClient] Failed to list workflows:', error);
            return []; // Return empty array if API unavailable
          }
        }
        ```
      ...
      - `async triggerWorkflow(workflowId: string, data: unknown): Promise<WorkflowTriggerResult>`
        ...
        async triggerWorkflow(workflowId: string, data: unknown): Promise<WorkflowTriggerResult> {
          ...
          try {
            ...
            const webhookPath = workflow?.settings?.webhookPath || `/webhook/${workflowId}`;
            ...
            const response = await backOff(
              ...
              }),
              {
                ...
                retry: (err, attemptNumber) => {
                  ...
                  console.warn(`[N8nWorkflowClient] triggerWorkflow attempt ${attemptNumber} failed:`, err.message);
                  return true;
                }
              }
            );
            ...
            if (!response.ok) {
              return {
                success: false,
                error: `Webhook trigger failed: ${response.status} ${response.statusText}`
              };
            }
      ...
      - `async triggerWorkflowByWebhook(webhookUrl: string, data: unknown): Promise<WorkflowTriggerResult>`
        - Direct POST to provided webhook URL with backOff retry
...
```
  ...
  <done>
N8nWorkflowClient created with listWorkflows, getWorkflow, getExecutionStatus, triggerWorkflow methods. Audit logging integrated. backOff retry wrapper implemented for all API calls.
  </done>
</task>
...
4. Error handling with backOff retry implemented (visible in listWorkflows and triggerWorkflow)
...
- backOff retry wrapper used for transient error handling
