...
type: execute
wave: 4
depends_on: ["06-01", "06-02", "06-03", "06-04", "06-05", "06-07"]
files_modified: []
autonomous: false
...
must_haves:
  truths:
    ...
    - "Audit logs capture all workflow AND graph data access operations"
...
Purpose: Validate that all components work together: n8n workflow client, MCP tools, webhook validation, audit logging (including retrofitted logging for existing tools), and execution tracking.
...
4. Verify audit logging is wired into all MCP tools:
   ```bash
   cd apps/omnii_mcp && grep -l "logAuditEvent" src/mcp/tools/*.ts | wc -l
   ```
   Should show 10 files (7 existing + 3 new workflow tools).
  </action>
  <verify>
Build succeeds without errors. All modules export expected functions and classes. All MCP tools have audit logging.
  </verify>
  <done>
Build verified. All new modules importable and functional. Audit logging confirmed in all tools.
  </done>
</task>
...
6. **Audit Logging Retrofit** (06-07)
   - Added audit logging to all 7 existing MCP tools
   - SEC-04 compliance for graph data access events
   - Consistent logging pattern across all tools
  </what-built>
  <how-to-verify>
...
6. **Verify audit logging on existing tools:**
   - Call any existing MCP tool (search_nodes, calendar_query, etc.)
   - Verify graph_data_accessed event appears in logs

7. **Verify Swagger documentation includes new endpoints:**
...
5. All MCP tools have audit logging (10 files with logAuditEvent)
6. n8n webhook endpoints operational
7. Human verification of end-to-end functionality
</verification>
...
- Phase 6 requirements met:
  ...
  - SEC-04: Audit logging for data access (Pino service + ALL MCP tools)
</success_criteria>
