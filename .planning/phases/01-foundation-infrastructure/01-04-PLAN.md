...
# Prior plan context (must be complete)
...
Note: Add `aura_instance_id` column to user_databases schema (text, nullable) for tracking.
  </action>
  <verify>
TypeScript compiles:
```bash
...
3. **Update user_databases schema** to include aura_instance_id:
```typescript
// In packages/db/src/schema.ts - add to UserDatabase table
