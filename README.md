...
> so you might need to disable that in [`@omnii/auth`](./packages/auth/src/index.ts)
...
> In this template, we use `@omnii` as a placeholder for package names. As a user, you might want to replace it with your own organization or project name. You can use find-and-replace to change all the instances of `@omnii` to something like `@my-company` or `@project-name`.
