# OMNII Terms of Service

**Effective Date:** May 30, 2025  
**Last Updated:** May 30, 2025

## 1. Acceptance of Terms

By downloading, installing, or using the OMNII mobile application ("App"), you agree to be bound by these Terms of Service ("Terms"). If you do not agree to these Terms, do not use the App.

These Terms constitute a legally binding agreement between you ("User" or "you") and OMNII Technologies ("Company," "we," "our," or "us").

## 2. Description of Service

### 2.1 OMNII App Features
OMNII is an AI-powered productivity assistant that provides:
- **AI Task Suggestions**: Intelligent task recommendations based on your productivity patterns
- **Approval System**: User-controlled approval process for all AI suggestions
- **Analytics Dashboard**: Productivity insights and performance tracking
- **Achievement System**: Gamified productivity milestones and progress tracking
- **Chat Interface**: Natural language interaction with AI assistant
- **Profile Management**: Personalized settings and preferences

### 2.2 AI-Powered Service Limitations
- AI suggestions are automated recommendations, not guaranteed productivity solutions
- Users maintain full control and decision-making authority over all suggestions
- AI accuracy may vary based on data quality and usage patterns
- Service functionality requires internet connectivity

## 3. User Accounts and Registration

### 3.1 Account Creation
- You must provide accurate information during registration
- You are responsible for maintaining account security and confidentiality
- You must be at least 13 years old to use the App
- One account per user; shared accounts are prohibited

### 3.2 Account Responsibilities
- You are responsible for all activity under your account
- Notify us immediately of any unauthorized access
- Use strong passwords and enable available security features
- Keep your contact information current

## 4. Acceptable Use Policy

### 4.1 Permitted Uses
- Use the App for personal productivity improvement
- Share approved content through designated sharing features
- Provide feedback to improve AI recommendations
- Export your own data as permitted by privacy controls

### 4.2 Prohibited Activities
- Reverse engineer, decompile, or attempt to extract source code
- Use the App for illegal, harmful, or unauthorized purposes
- Attempt to access other users' accounts or data
- Interfere with or disrupt the App's functionality
- Use automated systems to access the App without authorization
- Submit false, misleading, or malicious information
- Violate any applicable laws or regulations

## 5. Intellectual Property Rights

### 5.1 OMNII Intellectual Property
- The App and all related technology are owned by OMNII Technologies
- All trademarks, logos, and brand names are our exclusive property
- AI algorithms, user interface designs, and software code are proprietary
- You may not use our intellectual property without explicit written permission

### 5.2 User Content
- You retain ownership of content you create using the App
- You grant us a limited license to process your content for service provision
- You warrant that your content does not infringe third-party rights
- We may remove content that violates these Terms

### 5.3 Third-Party Content
- The App may include third-party integrations and content
- Third-party content is subject to separate license terms
- We do not endorse or guarantee third-party content accuracy

## 6. AI Service Terms

### 6.1 AI Functionality
- AI suggestions are based on machine learning algorithms and historical data
- AI recommendations are suggestions only, not professional advice
- Users must evaluate and approve all AI-generated suggestions
- AI accuracy and effectiveness may vary based on individual usage patterns

### 6.2 AI Limitations and Disclaimers
- AI suggestions are not substitutes for professional judgment
- We do not guarantee the accuracy or effectiveness of AI recommendations
- AI-generated content may contain errors or inaccuracies
- Users are solely responsible for decisions based on AI suggestions

### 6.3 AI Improvement
- Your usage data may be used to improve AI algorithms (anonymized)
- You can opt out of AI improvement data sharing in privacy settings
- AI model updates may change suggestion patterns over time

## 7. Privacy and Data Protection

### 7.1 Privacy Policy
- Your privacy is governed by our Privacy Policy, incorporated by reference
- Data collection and processing practices are detailed in the Privacy Policy
- You consent to data processing as described in the Privacy Policy

### 7.2 Data Security
- We implement industry-standard security measures
- You are responsible for maintaining your device and account security
- Report security concerns immediately to security@omnii.live

## 8. Payment Terms and Subscriptions

### 8.1 Free Service
- OMNII is currently provided as a free service
- We reserve the right to introduce paid features in the future
- Existing users will receive advance notice of any payment requirements

### 8.2 Future Paid Features (If Applicable)
- Payment terms will be clearly disclosed before implementation
- Subscription terms will be provided at the time of purchase
- Refund policies will comply with applicable app store policies

## 9. Service Availability and Modifications

### 9.1 Service Availability
- We strive to maintain continuous service availability
- Temporary interruptions may occur for maintenance or technical issues
- We do not guarantee uninterrupted access to the App

### 9.2 Service Modifications
- We may modify, update, or discontinue features at any time
- We will provide reasonable notice of material changes
- Continued use after changes constitutes acceptance of modifications

## 10. Limitation of Liability

### 10.1 Disclaimer of Warranties
- The App is provided "as is" without warranties of any kind
- We disclaim all warranties, express or implied, including merchantability and fitness for purpose
- We do not warrant that the App will be error-free or uninterrupted

### 10.2 Limitation of Damages
- Our liability is limited to the maximum extent permitted by law
- We are not liable for indirect, incidental, or consequential damages
- Total liability shall not exceed the amount paid by you for the service (currently $0)

### 10.3 Force Majeure
- We are not liable for delays or failures due to circumstances beyond our control
- This includes natural disasters, government actions, and third-party service failures

## 11. Indemnification

You agree to indemnify and hold harmless OMNII Technologies from any claims, damages, or expenses arising from:
- Your use of the App in violation of these Terms
- Your violation of any law or third-party rights
- Your content or data submitted through the App
- Unauthorized use of your account

## 12. Termination

### 12.1 Termination by You
- You may terminate your account at any time through the App settings
- Account termination will result in deletion of your data per our Privacy Policy
- You remain responsible for any obligations incurred before termination

### 12.2 Termination by Us
- We may terminate accounts for violations of these Terms
- We may suspend service for maintenance or security reasons
- We will provide notice when reasonably possible

### 12.3 Effect of Termination
- Upon termination, your right to use the App immediately ceases
- Data deletion will proceed according to our Privacy Policy
- Surviving provisions include intellectual property, liability limitations, and dispute resolution

## 13. Dispute Resolution

### 13.1 Governing Law
- These Terms are governed by the laws of [Jurisdiction]
- Any disputes will be resolved in the courts of [Jurisdiction]

### 13.2 Arbitration (If Applicable)
- Disputes may be subject to binding arbitration
- Arbitration terms will be specified in future updates if implemented
- You may opt out of arbitration requirements within 30 days of notice

### 13.3 Class Action Waiver
- Disputes must be resolved individually
- Class action lawsuits and class arbitrations are waived
- This waiver is subject to applicable law

## 14. Compliance and Legal Requirements

### 14.1 Export Control
- The App may be subject to export control laws
- You agree to comply with all applicable export regulations
- You represent that you are not located in a restricted jurisdiction

### 14.2 App Store Compliance
- Use of the App is subject to applicable app store terms
- App store terms supplement but do not replace these Terms
- Conflicts will be resolved in favor of the more restrictive terms

## 15. Accessibility and Non-Discrimination

### 15.1 Accessibility Commitment
- We strive to make the App accessible to users with disabilities
- The App includes VoiceOver support and Dynamic Type compatibility
- Report accessibility issues to accessibility@omnii.live

### 15.2 Equal Access
- We provide equal access to App features regardless of user characteristics
- We do not discriminate based on protected characteristics
- AI suggestions are based solely on productivity patterns, not personal characteristics

## 16. Contact Information and Support

### 16.1 General Contact
- **Email**: support@omnii.live
- **Website**: https://omnii.live/support
- **Privacy Questions**: privacy@omnii.live

### 16.2 Support Response Times
- **General Support**: Response within 48 hours
- **Security Issues**: Response within 24 hours
- **Accessibility Issues**: Response within 72 hours

### 16.3 Legal Notices
- **Legal Department**: legal@omnii.live
- **Copyright Notices**: copyright@omnii.live
- **Terms Questions**: terms@omnii.live

## 17. Changes to Terms

### 17.1 Modification Process
- We may update these Terms from time to time
- Material changes will be communicated through the App
- Continued use after changes constitutes acceptance

### 17.2 Notice Period
- We will provide at least 30 days notice for material changes
- Changes affecting user rights will receive prominent notification
- You may terminate your account if you disagree with changes

## 18. Severability and Entire Agreement

### 18.1 Severability
- If any provision is found unenforceable, the remainder remains in effect
- Unenforceable provisions will be modified to the minimum extent necessary
- The Terms will remain binding to the maximum extent permitted

### 18.2 Entire Agreement
- These Terms and our Privacy Policy constitute the entire agreement
- They supersede all prior agreements and understandings
- Modifications must be in writing and signed by both parties

---

**Effective Date**: May 30, 2025  
**Company**: OMNII Technologies  
**Contact**: legal@omnii.live

By using OMNII, you acknowledge that you have read, understood, and agree to be bound by these Terms of Service. 