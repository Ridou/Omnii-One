...
describe("Simple Schedule Test", () => {
  ...
  test("📅 Schedule a meeting for next week", async () => {
    if (!serverReady) {
      console.log("⏭️ Skipping test - service not ready");
      return;
    }

    console.log("📅 Testing schedule event scenario...");
    
    const message = "Schedule a meeting for next week";
    ...
    if (response.data?.structured?.ai_reasoning?.extracted_concepts) {
      ...
      // Check for schedule-related concepts
      const conceptNames = concepts.map((c: any) => c.concept_name.toLowerCase());
      const scheduleRelatedConcepts = conceptNames.filter(name => 
        name.includes('schedule') || name.includes('meeting') || name.includes('week') || name.includes('calendar')
      );
      console.log(`📅 Schedule-related concepts found: ${scheduleRelatedConcepts.length}`);
      expect(scheduleRelatedConcepts.length).toBeGreaterThan(0);
    }
    
    // Check structured actions for schedule actions
    if (response.data?.structured?.structured_actions) {
      ...
      // Look for schedule-related actions
      const scheduleActions = actions.filter((a: any) => 
        a.action_type === 'schedule_event' || a.action_type === 'create_task'
      );
      console.log(`📅 Schedule-related actions: ${scheduleActions.length}`);
    }
