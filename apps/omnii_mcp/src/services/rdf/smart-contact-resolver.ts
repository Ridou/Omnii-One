...
export class SmartContactResolver {
  ...
  private async searchWithMultipleTerms(
    ...
    for (const term of searchTerms) {
      ...
      const result = await this.trySearchVariant(term, context, "partial", userId);
      if (result.success) {
  ...
  private async tryExactSearch(
    ...
    const exactResult = await this.trySearchVariant(searchTerm, context, "exact", userId);
    if (exactResult.success) return exactResult;

    // Strategy 2: Case-insensitive partial search
    const partialResult = await this.trySearchVariant(searchTerm, context, "partial", userId);
    if (partialResult.success) return partialResult;

    // Strategy 3: Search with wildcards/broader query
    const broadResult = await this.trySearchVariant(searchTerm, context, "broad", userId);
  ...
  private hasValidContactData(result: any): boolean {
    const data = (result as any).rawData?.data || (result as any).rawData;
  ...
  private async getAllContacts(context: ExecutionContextType, userId?: string): Promise<any[]> {
    ...
    try {
      ...
      if (result.success && (result as any).rawData) {
        const data = (result as any).rawData.data || (result as any).rawData;
